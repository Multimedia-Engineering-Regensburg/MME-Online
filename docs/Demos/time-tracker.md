...
Einzelne Tasks werden in einer Liste (`<ul>`) des `tasks`-Element angezeigt. Verwenden Sie zur Darstellung der Listeneinträge die Templates aus den Elementen mit den ID `task-entry` und `task-label`. Wird eine gefilterte Liste dargestellt, wird am Ende der Liste das Element mit der Klasse `filter-info` eingeblendet, in dem die Anzahl der aktuell ausgeblendeten Elemente dargestellt wird.
