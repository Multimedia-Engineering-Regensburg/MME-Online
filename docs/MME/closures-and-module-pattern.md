...
Häufig wird das Objekt, das die Referenz auf den zu veröffentlichenden Teilbereich des Moduls enthält, nicht erst bei der Rückgabe erstellt sondern in einer lokalen Variable des  *Closures* gespeichert und anschließend um die öffentlichen Referenzen ergänzt. Dieses Objekt wird häufig in Anlehnung an das Schlüsselwort `this` mit dem Bezeichner `that` versehen:
